 ...
 */
	function year($fieldName, $minYear = null, $maxYear = null, $selected = null, $attributes = array(), $showEmpty = true) {
		...
		if (strlen($selected) > 4 || $selected === 'now') {
			...
			*/
			$selected = $this->Jtime->pdate('Y', strtotime($selected));
		} elseif ($selected === false) {
			$selected = null;
		}
		...
		if (!empty($selected)) {
			if (is_array($selected)) {
				...
				if (is_numeric($selected)) {
					...
					*/
					$selected = $this->Jtime->pdate('Y-m-d H:i:s', $selected);
				}
				$meridian = 'am';
				$pos = strpos($selected, '-');
				if ($pos !== false) {
 ...
 */
	function __generateOptions($name, $options = array()) {
		...
		switch ($name) {
			...
			case 'year':
				//FIX JALALI Year... 
				$current = intval($this->Jtime->pdate('Y'));
